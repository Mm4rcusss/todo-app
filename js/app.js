document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderSidebar() {
        ...
        state.lists.forEach((list) => {
            ...
            li.innerHTML = `
                <div class="list-info">
                    <span class="list-dot" style="background-color: ${escapeHtml(list.color || '#ffe135')};"></span>
    ...
    function bitsAccentFromParams(params) {
        return params.color || params.color1 || null;
    }
    ...
    function renderBitsControls() {
        ...
        schema.forEach((field) => {
            ...
            const applyValue = (raw) => {
                ...
                const accent = bitsAccentFromParams(state.bitsParams[themeId]);
                if (accent && (field.key === 'color' || field.key === 'color1')) {
    ...
    function applyTheme(themeId) {
        ...
        if (custom) {
            ...
            document.querySelector('meta[name="theme-color"]')?.setAttribute('content', custom.color);
        ...
        if (window.BitsFX?.isBitsTheme(themeId)) {
            const bits = window.BitsFX.themes.find((theme) => theme.id === themeId);
            const params = { ...window.BitsFX.getDefaults(themeId), ...(state.bitsParams[themeId] || {}) };
            const accent = params.color || params.color1 || bits.color;
            document.documentElement.style.setProperty('--color-accent', accent);
            document.querySelector('meta[name="theme-color"]')?.setAttribute('content', accent);
